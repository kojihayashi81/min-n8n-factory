...
test('buildSuccessMessage: webSkipReason=no_hints は「検索ヒントなし」ラベル付きで表示', () => {
  ...
  const section = findBlock(msg, 'section');
  assert.match(section.text.text, /検索ヒントなしによりスキップ/);
  assert.match(section.text.text, /80 点満点換算/);
});

test('buildSuccessMessage: webSkipReason=web_failed は「Web 調査が失敗したためスキップ」で表示', () => {
  const msg = buildSuccessMessage({
    ...COMMON,
    threadTs: '0',
    prUrl: 'https://github.com/owner/repo/pull/99',
  ...
  const section = findBlock(msg, 'section');
  assert.match(section.text.text, /Web 調査が失敗したためスキップ/);
});
...
test('elapsedSinceStart: epoch ms (number) を受け付ける', () => {
  const startMs = Date.now() - 200 * 1000;
  const elapsed = elapsedSinceStart(startMs);
  assert.ok(elapsed >= 200 && elapsed <= 202);
});

test('elapsedSinceStart: 0 (number) は 0 ではなく epoch=0 として扱われる', () => {
  // 0 is a valid epoch timestamp (1970-01-01), not a sentinel.
  // The elapsed time should be huge, not 0.
  const elapsed = elapsedSinceStart(0);
  assert.ok(elapsed > 0);
});
...
test('buildPayloadForContext: kind=success で epoch ms (number) の executionStartedAt が経過時間に反映される', () => {
  const startMs = Date.now() - 192 * 1000;
  const payload = buildPayloadForContext({
    kind: 'success',
    issue: ISSUE,
    env: ENV,
  ...
  const ctx = findBlock(payload, 'context');
  // 3分12秒 = 192秒 (±2秒の実行ずれ許容)
  assert.match(ctx.elements[0].text, /⏱️ 3分1[0-4]秒/);
});

test('buildPayloadForContext: kind=failure で epoch ms (number) の executionStartedAt が経過時間に反映される', () => {
  const startMs = Date.now() - 601 * 1000;
  const payload = buildPayloadForContext({
    kind: 'failure',
    issue: ISSUE,
    env: ENV,
  ...
  const ctx = findBlock(payload, 'context');
  assert.match(ctx.elements[0].text, /⏱️ 10分0[0-3]秒/);
});
