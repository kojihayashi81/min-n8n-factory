#!/bin/bash
# n8n-run-claude-pipeline.sh — 4-stage agent pipeline for issue investigation.
#
# Caller: n8n "Run Claude Code" node (ExecuteCommand)
#   /opt/scripts/n8n-run-claude-pipeline.sh <issue-number>
#
# Stdout on success:
#   QUALITY_SCORE=<score>/<max>                  (when Gatekeeper ran)
#   QUALITY_SCORE_RERUN=<score>/<max>            (only after Synthesizer rerun)
#   WEB_SKIP_REASON=<no_hints|web_failed>        (only when Web investigation was skipped)
#   https://github.com/<owner>/<repo>/pull/<n>   (Draft PR URL)
#
# On failure: non-zero exit + stderr contains failed agent name / stdout / stderr
# with delimiters.
#
# Design details: docs/workflows/ai-issue-processor/agent_pipeline.md
set -euo pipefail
...
# Determine pass/fail deterministically on the shell side. The Gatekeeper
# prompt includes the threshold so the agent fills in `.pass` itself, but
# hallucination can produce pass=true even when score < threshold. Rely on
# the numeric comparison of SCORE vs PASS_THRESHOLD instead.
