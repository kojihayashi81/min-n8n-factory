...
| `Run Claude Code` | `/opt/scripts/n8n-run-claude.sh` を呼び出して Claude Code で調査を実行。成功時は claude の stdout（通常は Draft PR URL を含む）を素通しで返す。失敗時はスクリプト内で `set +e` / `set -e` で `timeout … claude …` を囲み、claude の stdout / stderr を個別 temp file に退避した上でスクリプト stderr にデリミタ付きで吐き出し、claude の exit code をそのまま `exit` する。n8n ExecuteCommand ノードは失敗時に `error` / `stderr` フィールドへその内容を格納するので、後段の `Build failure payload` → `resolveFailureError` に実際の失敗理由（auth エラー、ツール失敗、レートリミット等）が到達する |
...
- **Claude Code タイムアウト**: `CLAUDE_TIMEOUT_SEC`（デフォルト 600秒）。`n8n-run-claude.sh` 内で `timeout(1)` 経由で制御し、超過時は exit code 124 + claude の部分 stderr をそのまま n8n に伝搬する（Slack 失敗通知の本文に実際の timeout 原因が出る）
...
失敗時の Slack メッセージおよび GitHub Issue エラーコメントには、**Claude Code の実際の stderr / exit code が秘匿スクラブ済みで**そのまま載る。
...
1. `n8n-run-claude.sh` が stdout / stderr を個別 temp file に退避
2. `set +e` / `set -e` で `timeout … claude …` を囲み、exit code を安全に捕捉
3. 失敗時はスクリプト stderr に両ストリームをデリミタ付きで吐いた上で、claude の exit code をそのまま `exit`
