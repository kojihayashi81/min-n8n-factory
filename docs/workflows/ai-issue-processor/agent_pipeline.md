...
Gatekeeper（軽量）
  ├─ pass (score が最大値の 70% 以上) → PR 作成 + Slack 通知
  └─ fail (score が最大値の 70% 未満)  → Synthesizer を feedback 付きで 1 回だけ再実行
                                         → Gatekeeper 再実行（通知用スコア取得のみ）
                                         → PR 作成 + Slack 通知
```

通常ケースのスコアは 100 点満点で 70 点以上が pass。Web Investigator を失敗・スキップしたケースのみ 80 点満点換算（pass 閾値 56 点）になる。詳細は [Agent 4: Gatekeeper](#agent-4-gatekeeper) と [再実行ポリシー](#再実行ポリシー) を参照。
...
調査の本体。Code → Web の **直列構成** で、Code Investigator の出力した `search_hints` が Web Investigator の検索クエリを決定する。

並列化案（Code と Web を並走させ、Collector の `initial_keywords` を Web の初期クエリに使う）はコスト同じで wall-clock を削減できるが、Issue 文面だけから生成した `initial_keywords` では検索精度が不足しやすく、Code Investigator が実際の実装を読んで導出した `search_hints`（特定のライブラリ名・バージョン・関数名を含む）のほうが Web 検索の解像度が大きく上がる。調査ノートの品質差は wall-clock 短縮よりも大きいと判断して、あえて直列構成を採用している。
...
| max-turns | 制限なし（コードベースの規模に依存） [^max-turns] |
| 制約 | Web 検索を行わない。実装コードを書かない |

[^max-turns]: `--max-turns` 未指定＝ターン数の上限なし。実質的な停止条件は当該エージェントの個別タイムアウト（Code Investigator 300 秒 / Synthesizer 180 秒）のみ。運用時に「無制限で動き続ける」と誤読しないよう注意。エージェントごとの個別タイムアウトは [エージェントごとのタイムアウト](#エージェントごとのタイムアウト) を参照。
...
| max-turns | 3 |
| 制約 | コードベースを読まない。`search_hints` を優先的に検索する。Collector が抽出した `linked_urls` は `WebFetch` で直接読みに行く |
...
| max-turns | 制限なし（[^max-turns] と同じ扱い。実質制限は Synthesizer の個別タイムアウト 180 秒） |
...
**pass 閾値の根拠**: 20 点満点 × 5 項目 = 100 点満点。pass 閾値 **70 点** は各項目平均 14 点 = 70% ラインを意味する。初期運用は合計閾値のみで判定し、「根拠の具体性 8 点 × テンプレート準拠 20 点 = 合計 72 点で pass」のように特定項目が壊滅的に低いのに合計で pass するエッジケースは許容する。運用ログを見て問題が再発するようなら、項目別下限（例: テンプレート準拠 10 点未満で即 fail）を追加する。

**Web 調査失敗 / スキップ時の採点**: Web Investigator が失敗した場合、または Collector の `search_hints` が空で Web 調査を実行しなかった場合、シェルスクリプトは Gatekeeper の入力に `web_status=skipped` フラグを渡す。このときは **採点項目 5（テンプレート準拠）を除外** して **80 点満点換算** で採点し、pass 閾値は **56 点**（80 × 0.7）となる。理由は、Web 調査の成果である「外部参考ソース URL」を要求する項目 5 は Web 失敗時には構造的に満点を取れず、Synthesizer 再実行しても改善不能という矛盾を避けるため。
...
- Gatekeeper の `pass` が `false`（score が pass 閾値未満）の場合、Synthesizer を `feedback` 付きで **1 回だけ** 再実行する
- 再実行時の Synthesizer は、元の入力（Collector + Code + Web の各出力）に加えて Gatekeeper の `feedback` を受け取り、指摘箇所を改善した調査ノートを再生成する
- 再実行後は **Gatekeeper をもう 1 回だけ走らせる**。2 回目のスコアは閾値判定には使わず、通知と観測のみに使う（再実行の効果をメトリクスとして追跡するため）
- 2 回目の Gatekeeper スコアがどうであっても PR 作成と Slack 通知に進む。`Synthesizer 再々実行` は行わない（無限ループ防止）
...
パイプライン全体のタイムアウトは `CLAUDE_TIMEOUT_SEC`（デフォルト 1020 秒）で制御し、各エージェントには個別のタイムアウトを持たせる。
...
以下の表は **1.0x = 既存の単発 `n8n-run-claude.sh` 1 回実行相当のトークン消費量（入力 + 出力の合計）** を基準とした相対値[^cost-baseline]。
...
| Gatekeeper 再実行 | 0.1x | Synthesizer 再実行時のみ。通知用スコアの取得に使う |

通常ケース（再実行なし）: 約 **1.4x**。再実行ありのケース: 約 **1.9x**（Gatekeeper 再実行 +0.1x 込み）。

単発実行と比較してコスト増は 1.4〜1.9 倍だが、各エージェントのコンテキストウィンドウが小さい（不要なツール結果が混入しない）ため、実際のトークン消費はこの見積もりより低くなる可能性がある。

[^cost-baseline]: 基準をドル換算ではなくトークン量に取っているのは、将来モデルが変わっても相対比率が崩れにくいため。実際の金額影響は「1.0x の単発実行が何トークン × 使用モデルの単価」で換算する。
