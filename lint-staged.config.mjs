const hasScriptsChange = (files) =>
  files.some((f) => /(^|\/)scripts\//.test(f));

const hasMcpSrcChange = (files) =>
  files.some((f) => /(^|\/)mcp-server\/src\//.test(f));
...
export default (stagedFiles) => {
  ...
  const fmtFiles = stagedFiles.filter((f) =>
    /\.(js|ts|json|yml|yaml)$/.test(f),
  );
  if (fmtFiles.length) {
