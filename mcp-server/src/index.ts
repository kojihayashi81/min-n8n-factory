...
function createServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      title: 'プロジェクト知識検索',
      description: 'ドキュメントと実装要約を横断検索する。キーワードで関連するリソースを返す。',
      inputSchema: {
    ...
    async ({ query, scope }) => {
      ...
      const text = results
        .slice(0, 10)
        .map((r) => {
          const gaps = r.knownGaps.length > 0 ? `\n  注意: ${r.knownGaps.join('; ')}` : '';
  ...
  server.registerTool(
    'explain_project_topic',
    {
      ...
      inputSchema: {
        ...
        includeImplementation: z.boolean().default(true).describe('実装由来の補足を含めるか'),
      },
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
    ...
    async ({ topic, includeImplementation }) => {
      ...
      if (result.specSections.length === 0 && result.derivedSections.length === 0) {
        return {
          content: [
            {
              type: 'text' as const,
    ...
    'detect_doc_impl_drift',
    {
      ...
      inputSchema: {
        area: z
          .string()
          .optional()
          .describe('検出対象の絞り込み（例: ラベル, make, ワークフロー）。省略時は全領域。'),
      },
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
...
async function main() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[mcp-docs] MCP server listening on http://127.0.0.1:${PORT}/mcp`);
  });
}
