...
function describeFlow(wf: WorkflowDef): string {
  ...
  for (const [from, targets] of Object.entries(wf.connections)) {
    try {
      ...
      console.warn(
        `[pipeline] Unexpected connection structure for node "${from}" in ${wf.fileName}`
      );
    }
  }
  return lines.join('\n');
...
function buildOne(wf: WorkflowDef, generatedAt: string): ResourceEntry {
  ...
  const nodeList = wf.nodes.map((n) => `- **${n.name}** (\`${n.type}\`)`).join('\n');
...
export function buildWorkflowSummaries(defs: WorkflowDef[], generatedAt: string): ResourceEntry[] {
