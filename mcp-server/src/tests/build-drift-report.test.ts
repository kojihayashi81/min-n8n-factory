...
function makeWorkflowWithLabels(labels: Record<string, string>): WorkflowDef[] {
...
describe('buildDriftReport', () => {
  it('detects labels in impl but not in spec', () => {
    ...
    const labelDrift = items.filter((i) => i.area === 'ラベル定義' && i.severity === 'warning');
  ...
  it('detects labels in spec but not in impl', () => {
    ...
    const { items } = buildDriftReport(labelDefs, [], [], workflows, NOW);
    const unused = items.filter((i) => i.area === 'ラベル定義' && i.severity === 'info');
  ...
  it('detects make targets in README but not in Makefile', () => {
    ...
    const makeDrift = items.filter((i) => i.area === 'make コマンド' && i.severity === 'warning');
  ...
  it('detects undocumented make targets', () => {
    ...
    const { items } = buildDriftReport([], specDocs, makeTargets, [], NOW);
    const undocumented = items.filter((i) => i.area === 'make コマンド' && i.severity === 'info');
