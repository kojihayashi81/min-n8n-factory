...
describe('loadMakefile', () => {
  ...
  it('resets comment on blank line', async () => {
    await fs.writeFile(
      path.join(tmpDir, 'Makefile'),
      ['# This comment is orphaned', '', 'target:', '\techo hello'].join('\n')
    );
  ...
  it('handles target with no comment', async () => {
    await fs.writeFile(path.join(tmpDir, 'Makefile'), ['build:', '\tnpm run build'].join('\n'));
  ...
  it('joins multiple comment lines', async () => {
    await fs.writeFile(
      path.join(tmpDir, 'Makefile'),
      ['# Line 1', '# Line 2', 'multi:', '\techo multi'].join('\n')
    );
